...
# Make sure Brand Code is a factor
train_data$`Brand Code` <- as.factor(train_data$`Brand Code`)

# Run kNN only on the specific variables that have missing values,
# and explicitly exclude Brand Code
vars_to_impute <- names(train_data)[names(train_data) != "Brand Code"]

train_data_imputed <- kNN(
  train_data,
...
# Double-check Brand Code
table(train_data_imputed$`Brand Code`)
